...
const { loadBestScore, saveBestScore, resetBestScore } = createBestScoreStore("dialed.bestScore");

/* ── 37apps brand kit: light neutral base + Amber accent ── */
const palette = {
  ...baseTheme,
  track: "#EFEAE1",
  trackBorder: "#D6D0C2",
...
/* Dialed's fixed brand accent (Jade Flash) — drives PLAY/RETRY buttons and the
...
/* Difficulty ramps hold flat for a short grace window so new players get a
   few free reps, then climb gently — a slow, gradual curve reads as "hard to
   master" instead of "hard to survive round two". */
const SPEED_GRACE_SCORE = 3;
const BASE_ANGULAR_SPEED = 2.0;
const ANGULAR_SPEED_PER_SCORE = 0.07;
const MAX_ANGULAR_SPEED = 5.2;

const ZONE_GRACE_SCORE = 4;
const START_ZONE_WIDTH = 100;
const SHRINK_PER_SCORE = 2.2;
const MIN_ZONE_WIDTH = 30;
...
function zoneWidthForScore(score) {
  const eased = Math.max(0, score - ZONE_GRACE_SCORE);
  return Math.max(MIN_ZONE_WIDTH, START_ZONE_WIDTH - eased * SHRINK_PER_SCORE);
}

function angularSpeedForScore(score) {
  const eased = Math.max(0, score - SPEED_GRACE_SCORE);
  return Math.min(MAX_ANGULAR_SPEED, BASE_ANGULAR_SPEED + eased * ANGULAR_SPEED_PER_SCORE);
}
...
export default function Dialed() {
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      ...
      if (now >= frozenUntilRef.current) {
        ...
        if (frameCountRef.current % TRAIL_SAMPLE_EVERY === 0) {
          const offset = AMPLITUDE * Math.sin(phaseAngleRef.current);
          const trail = trailRef.current.concat([offset]);
          trailRef.current = trail.length > TRAIL_LENGTH ? trail.slice(trail.length - TRAIL_LENGTH) : trail;
        }
      }
  ...
  /* ambient glow reacts to streak so the backdrop tells you you're on a run
     without another number on screen */
  const glowOpacity = Math.min(0.32, 0.05 + streak * 0.035);
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
        {phase === "play" && (
          <>
            {/* ambient glow: a soft accent blob behind the dial, growing with
                streak — ties the backdrop to the run instead of sitting static */}
            <div style={{
              position: "absolute", left: "50%", top: "50%",
              width: 340, height: 340, borderRadius: "50%",
              transform: "translate(-50%, -50%)",
              background: palette.perfect, opacity: glowOpacity,
              filter: "blur(70px)", transition: "opacity 0.3s ease",
              pointerEvents: "none",
            }} />

            <div style={{ position: "relative", width: BAR_WIDTH, height: MARKER_SIZE * 2 }}>
              {/* dial ticks: a row of gauge marks under the track, the game's
                  namesake motif — turns a plain bar into a dial face */}
              <div style={{
                position: "absolute", top: "50%", left: 0, right: 0,
                height: 20, transform: "translateY(2px)",
                ...
                {Array.from({ length: TICK_COUNT }).map((_, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      width: 2, height: isCenter ? 12 : 7,
                      background: palette.tick, borderRadius: 1,
                      opacity: isCenter ? 0.9 : 0.55,
                    }} />
                  );
                })}
              </div>
              ...
              {/* motion trail: fading echoes of the needle's last few
                  positions, reads the sweep as rhythm rather than a jump */}
              {trailRef.current.map((offset, i) => (
                <div key={i} style={{
                  position: "absolute", top: "50%",
                  left: BAR_WIDTH / 2 + offset,
                  width: MARKER_SIZE * 0.55, height: MARKER_SIZE * 0.55,
                  transform: "translate(-50%, -50%)",
                  background: palette.marker, borderRadius: "50%",
                  opacity: (0.08 + (i / TRAIL_LENGTH) * 0.22),
                  pointerEvents: "none",
                }} />
              ))}
              ...
              {/* perfect ping: an expanding ring at the needle on a dead-center hit */}
              {flash === "perfect" && (
                <div style={{
                  position: "absolute", top: "50%",
                  left: BAR_WIDTH / 2 + markerOffset,
                  width: MARKER_SIZE, height: MARKER_SIZE,
                  transform: "translate(-50%, -50%)",
                  border: `2px solid ${palette.perfect}`, borderRadius: "50%",
                  animation: "ringPing 0.5s ease-out forwards",
                  pointerEvents: "none",
                }} />
              )}
            </div>
        ...
        {phase === "start" && (
          <StartScreen
            ...
            description="Tap when the needle is inside the zone. Land dead center for a PERFECT — chain perfects for a streak bonus. Miss the zone and it's over."
            best={best}
            onPlay={startGame}
